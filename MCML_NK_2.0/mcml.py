...
class MCML():
    def __init__(self, cnf: dict, obj: list) -> float:
        """
    ...
    def get_func_collimated_distribution(self):
        """
        Возвращаем функцию, которая возвращает случайные (нет) направляющие косинусы фотона, в соответсвии с коллимированным излучением 
        """
        @njit(fastmath=True)
        def buf():
            return 0, 0, 1
    ...
    def parse_mode_save(self):
        ...
        save_obj - возвращает объект хранения данных (нужно в нескольких местах создавать такие объекты, todo отдельный класс?)
    ...
    #output
    def get_output(self):
        # Хе, ну save_interpreter() это типо внутренняя функция, а гет отпут это для людей, забыл как оно делается нормально
